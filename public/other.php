...
define("main/", "http" . (isset($_SERVER['HTTPS']) ? "s" : "") . "://" . $_SERVER["SERVER_NAME"] . ($_SERVER["SERVER_PORT"] != 80 ? ":" . $_SERVER["SERVER_PORT"] : "") . $_SERVER["SCRIPT_NAME"] . "/");
...
function proxifyCSS($css, $baseURL) {
  return preg_replace_callback(
    ...
    function($matches) use ($baseURL) {
        ...
        return "url(" . main/ . rel2abs($url, $baseURL) . ")";
    },
    $css);
}
...
<body>
    <form onsubmit="window.location.href='main/' + document.getElementById('site').value; return false;">
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query('//form') as $form) {
    ...
    $form->setAttribute("action", main/ . $action);
  }
  //Profixy <style> tags.
